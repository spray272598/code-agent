...
func (r *SQLiteUserRepo) FindByEmail(ctx context.Context, email string) (*auth.User, error) {
	return scanUserSQLite(r.db.QueryRowContext(ctx, userColsSQLite+` WHERE email=?`, email))
}
...
const userColsSQLite = `SELECT id,email,password_hash,display_name,role,status,email_verified,verify_token,reset_token,reset_expires_at,quota_tokens,quota_reset_at,created_at,updated_at FROM users`
...
const deviceColsSQLite = `SELECT id,user_code,user_id,status,scope,expires_at,last_poll_at,approved_at,created_at FROM devices`
...
ON CONFLICT(id) DO UPDATE SET user_id=excluded.user_id, status=excluded.status, last_poll_at=excluded.last_poll_at, approved_at=excluded.approved_at`,
		d.ID, d.UserCode, d.UserID, d.Status, d.Scope, sqliteTime(d.ExpiresAt), sqliteTime(d.LastPollAt), sqliteTime(d.ApprovedAt), sqliteTime(nowOr(d.CreatedAt)))
	return err
}
...
const refreshColsSQLite = `SELECT id,user_id,device_id,token_hash,scope,expires_at,revoked,created_at,updated_at FROM refresh_tokens`
...
ON CONFLICT(id) DO UPDATE SET user_id=excluded.user_id, device_id=excluded.device_id,
  token_hash=excluded.token_hash, scope=excluded.scope, expires_at=excluded.expires_at, revoked=excluded.revoked, updated_at=excluded.updated_at`,
		t.ID, t.UserID, t.DeviceID, t.TokenHash, t.Scope, sqliteTime(t.ExpiresAt), boolToInt(t.Revoked), sqliteTime(nowOr(t.CreatedAt)), time.Now().Format(time.RFC3339Nano))
	return err
}
